...
private:
    ...
    bool ShouldSkipRef(const NameReferenceExpr& ref, const Decl* target) const;

    std::string GetRefIdentifier(const Decl& target) const;

    bool CalculateRefPosition(const NameReferenceExpr& ref, const Decl& target,
                              SymbolLocation& loc) const;

    void AddRefToMap(const Decl& target, const Ref& refInfo);

    // Check if ref is inside @Component/@Entry macro-expanded code
    bool ShouldSkipMacroRef(const NameReferenceExpr& ref) const;
    // Get the current function declaration from scope stack
    const FuncDecl* GetCurrentFuncDecl() const;
    // Check if a function is macro-generated by its context (curMacroCall)
    static bool IsMacroGeneratedByContext(const FuncDecl* funcDecl, const std::string& funcName);

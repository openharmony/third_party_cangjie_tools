...
inline Cangjie::Position GetEnd(Ptr<Node> n) { return n->end; }
} // namespace
...
int ASTToFormatSource::IsMultipleMethodChain(const MemberAccess& memberAccess)
{
    if (memberAccess.baseExpr->astKind == ASTKind::CALL_EXPR) {
        ...
        if (ce->baseFunc && ce->baseFunc->astKind == ASTKind::MEMBER_ACCESS) {
            ...
            return 1 + IsMultipleMethodChain(*ma);
        } else {
            return 1;
        }
